...
export default async function ImpactPage() {
  ...
  return <main id="conteudo"><PageHero eyebrow="Resultados que apontam caminhos" title="Quando a oportunidade chega, o talento responde." description="Números informados pelo Projeto Pitagóricos sobre sua atuação desde 2022."/>
    ...
    <section className="section"><div className="shell"><div className="section-heading"><p className="eyebrow">Nossa trajetória</p><h2>Uma rede em expansão.</h2></div><div className="timeline">{timeline.map((item) => <article key={item.year}><span>{item.year}</span><div><h2>{item.title}</h2><p>{item.description}</p></div></article>)}</div></div></section>
