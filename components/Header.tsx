...
export function Header() {
  ...
  const authLinks = <>{!user && <Link className="button button-yellow compact" href="/entrar">Entrar</Link>}{user && <><Link className="button button-yellow compact" href="/area-do-aluno">Área do aluno</Link>{profile?.role === "admin" && <Link className="button button-outline-light compact" href="/admin">Admin</Link>}<button className="header-logout" onClick={() => void signOut()}>Sair</button></>}</>;
