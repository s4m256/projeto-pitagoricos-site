...
A versão branca é usada no header, hero, footer e na prévia social, todos sobre fundo escuro. A versão azul é usada nos cartões claros de autenticação. O excesso de canvas é recortado somente na apresentação por CSS; os pixels dos arquivos originais, a proporção, a arte e as cores permanecem intactos. Para evitar emendas visíveis, os fundos ao redor convergem para as cores exatas dos canvases oficiais (`#021C4D` no escuro e `#FEFEFE` no claro).
